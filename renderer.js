...
downloadBtn.addEventListener('click', () => {
  let rawInput = shareCodeInput.value.trim();
  const downloadPath = downloadFolderInput.value;
  let shareCode = rawInput;

  // **FIX**: Extract the share code from a full Steam link
  if (rawInput.includes('CSGO-')) {
    const startIndex = rawInput.indexOf('CSGO-');
    shareCode = rawInput.substring(startIndex);
  }

  if (!shareCode || !downloadPath) {
    addStatusMessage('Please provide a valid share code and a download folder.', 'error');
    return;
  }
